...
-- "Baraja nueva" para testers (solo Tienda, solo modo_tester): reemplaza cartas
-- 'disponible' y 'cumplida' por cartas nuevas sin esperar los 7 días. No toca
-- 'jugada' (ya se la mandaste a tu pareja) ni 'bloqueada'/'robada' (llave foránea
-- desde plot_twists_desbloqueados).
...
declare
  ...
  v_nuevo uuid;
begin
  select pareja_id, modo_spicy_activo, coalesce(modo_tester, false)
    into v_pareja, v_spicy, v_tester
  from usuarios where id = v_uid;
  if v_pareja is null then raise exception 'SIN_PAREJA'; end if;
